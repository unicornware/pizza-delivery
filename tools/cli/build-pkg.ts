...
async function buildPackage(): Promise<void> {
  ...
  return void (await build<BuildPkgOptions>(opts, async (argv, context) => {
    ...
    for (const format of formats) {
      ...
      if (format !== 'types') {
        // Get bundle name
        const bundle = npm_package_name.split('/')[1]!

        // Create bundles
        // See: https://github.com/vercel/ncc
        const BUNDLES = [bundle, `${bundle}.min`].map(async name => {
          const filename = `${format}/${name}.${extension}`
          const entrypoint = 'src/index.ts'
          const esm = format === 'esm'

          if (!dryRun) {
            const { code } = await ncc(`${cwd}/${entrypoint}`, {
              esm,
              externals: [
            ...
            await fs.writeFile(filename, code, { flag: 'wx+' })
